...
- Phase 9 mở rộng nền tảng — TikTok lát đầu đã thêm:
  ...
  - TikTok video publish có 2 mode thật: Direct Post qua `/v2/post/publish/video/init/` với scope `video.publish`, hoặc Upload to TikTok Inbox qua `/v2/post/publish/inbox/video/init/` với scope `video.upload`. Cả hai mode upload `FILE_UPLOAD` theo chunk bằng `upload_url`; OAuth request trực tiếp các scope posting + `video.list`, nếu app thiếu scope thì hiển thị lỗi cấu hình cho user/admin.
  ...
  - Adapter có `video.list`/`video.query` để đọc list/video metrics; OAuth request `video.list` mặc định vì app production đã cấu hình scope này.
