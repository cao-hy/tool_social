...
- Phase 9 mở rộng nền tảng — TikTok lát đầu đã thêm:
  ...
  - TikTok video publish có 2 mode thật: Direct Post qua `/v2/post/publish/video/init/` với scope `video.publish`, hoặc Upload to TikTok Inbox qua `/v2/post/publish/inbox/video/init/` với scope `video.upload`. Cả hai mode upload `FILE_UPLOAD` theo chunk bằng `upload_url`; mặc định OAuth chỉ request `video.upload`, bật `TIKTOK_ENABLE_DIRECT_POST_SCOPE=true` khi app đã được cấp Direct Post.
